...
const ProjectCard = memo(function ProjectCard({ 
  ...
  const endInfo = useMemo(() => {
    ...
    const isOngoing = project.endDate?.toLowerCase() === 'present';
    const end = isOngoing ? new Date() : (project.endDate ? new Date(project.endDate) : new Date());
    const endText = isOngoing ? 'Present' : new Intl.DateTimeFormat(undefined, { 
      year: 'numeric', 
      month: 'short' 
    }).format(end);
